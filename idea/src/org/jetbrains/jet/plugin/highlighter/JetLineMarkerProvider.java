...
public class JetLineMarkerProvider implements LineMarkerProvider {
    ...
    private static final MarkerType OVERRIDDEN_PROPERTY = new MarkerType(
            new NullableFunction<PsiElement, String>() {
                @Override
                public String fun(@Nullable PsiElement element) {
                    List<PsiMethod> methods = getPropertyMethods(element);
                    return !methods.isEmpty() ? StringUtil.join(
                            Collections2.transform(methods, new com.google.common.base.Function<PsiMethod, String>() {
                                @Override
                                public String apply(@Nullable PsiMethod method) {
                                    assert method != null;
            ...
            new LineMarkerNavigator() {
                ...
                public void browse(@Nullable MouseEvent e, @Nullable PsiElement element) {
                    ...
                    if (element instanceof PsiMethod) {
                        MarkerType.navigateToOverriddenMethod(e, (PsiMethod) element);
                        return;
                    }

                    if (element.getParent() instanceof JetProperty) {
                        ...
                        if (!ProgressManager.getInstance().runProcessWithProgressSynchronously(
                                ...
                                true,
                                element.getProject(), e != null ? (JComponent) e.getComponent(): null)) {
    ...
    @NotNull
    private static List<PsiMethod> getPropertyMethods(@Nullable PsiElement element) {
        if (element == null) return Collections.emptyList();
        if (element instanceof PsiMethod) return new SmartList<PsiMethod>((PsiMethod) element);

        if (element.getParent() instanceof JetProperty) {
            return Lists.newArrayList(LightClassUtil.getLightClassPropertyMethods((JetProperty) element.getParent()));
        }
